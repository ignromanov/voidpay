...
- **History:** Массив метаданных созданных инвойсов.
  - Структура: `{ id, clientName, amount, date, link, txHash?, paidAt? }`.
  - **Background Polling:** Приложение автоматически проверяет статус неоплаченных инвойсов из истории (где нет `txHash`), используя Alchemy Transfers API, и обновляет запись при обнаружении платежа.
