...
- [ ] T001 Install project dependencies (if not already present): `@upstash/ratelimit`, `@vercel/kv` for rate limiting
- [ ] T002 [P] Create feature directory structure at `src/features/rpc-proxy/`
- [ ] T003 [P] Create types file at `src/features/rpc-proxy/model/types.ts`
...
- [ ] T004 Create RPC configuration loader in `src/features/rpc-proxy/lib/config.ts` with environment variable validation
- [ ] T005 [P] Implement type definitions for RPC requests/responses in `src/features/rpc-proxy/model/types.ts` based on data-model.md
- [ ] T006 [P] Add environment variable schema to `src/shared/config/env.ts` for Alchemy/Infura API keys and Vercel KV credentials
- [ ] T007 Create Edge API route file at `src/app/api/rpc/route.ts` with basic POST handler structure
...
- [ ] T008 [P] [US1] Implement proxy logic with failover in `src/features/rpc-proxy/lib/proxy.ts` - handle primary provider request
- [ ] T009 [P] [US1] Implement provider configuration types and factory in `src/features/rpc-proxy/lib/config.ts` - create Alchemy and Infura provider configs
- [ ] T010 [US1] Add failover logic to proxy in `src/features/rpc-proxy/lib/proxy.ts` - detect errors/timeouts and retry with fallback provider
- [ ] T011 [US1] Implement timeout handling (2 second failover threshold) in `src/features/rpc-proxy/lib/proxy.ts`
- [ ] T012 [US1] Add error classification logic in `src/features/rpc-proxy/lib/proxy.ts` - distinguish between retryable and non-retryable errors
- [ ] T013 [US1] Integrate proxy logic into Edge API route in `src/app/api/rpc/route.ts` - wire up request handling
- [ ] T014 [US1] Add HTTP 503 error response for complete provider failure in `src/app/api/rpc/route.ts`
- [ ] T015 [US1] Add user-friendly error messages with retry guidance in `src/app/api/rpc/route.ts`
...
- [ ] T016 [P] [US2] Implement CORS configuration in `src/app/api/rpc/route.ts` - restrict to application domain only
- [ ] T017 [P] [US2] Add HTTP method validation in `src/app/api/rpc/route.ts` - accept only POST requests
- [ ] T018 [P] [US2] Implement JSON-RPC method allowlist validation in `src/features/rpc-proxy/lib/proxy.ts` using RpcMethod type from contracts
- [ ] T019 [US2] Add Origin and Referer header verification in `src/app/api/rpc/route.ts`
- [ ] T020 [US2] Implement anonymous request ID generation in `src/features/rpc-proxy/lib/proxy.ts` for operational metrics (no user linkage)
- [ ] T020.1 [US2] Add rate limit response headers (X-RateLimit-Limit, X-RateLimit-Remaining) as passive operational metrics (FR-020)
- [ ] T021 [US2] Add security headers (no-cache, no-store) to responses in `src/app/api/rpc/route.ts`
- [ ] T022 [US2] Verify no console.log or telemetry calls exist in proxy logic - code audit of `src/features/rpc-proxy/`
- [ ] T023 [US2] Add environment variable validation to ensure API keys are never exposed in client bundles
...
- [ ] T024 [P] [US3] Create mock provider implementation in `src/features/rpc-proxy/lib/mock.ts` with basic structure
- [ ] T025 [P] [US3] Implement mock detection logic in `src/app/api/rpc/route.ts` - check for localhost or debug query param
- [ ] T026 [US3] Add simulation mode parsing in `src/features/rpc-proxy/lib/mock.ts` - handle `?mock=success|error|slow` query params
- [ ] T027 [US3] Implement mock handlers for `eth_blockNumber`, `eth_call`, `eth_getBalance` in `src/features/rpc-proxy/lib/mock.ts`
- [ ] T028 [US3] Implement mock handlers for `eth_sendRawTransaction`, `eth_getTransactionReceipt` in `src/features/rpc-proxy/lib/mock.ts`
- [ ] T029 [US3] Add fake transaction hash generation logic in `src/features/rpc-proxy/lib/mock.ts` - use crypto.randomUUID() with hex prefix
- [ ] T030 [US3] Implement realistic delay simulation in `src/features/rpc-proxy/lib/mock.ts` - 1-3s for success, 10-30s for slow mode
- [ ] T031 [US3] Add mock error responses in `src/features/rpc-proxy/lib/mock.ts` - simulate various JSON-RPC error codes
- [ ] T032 [US3] Integrate mock provider into Edge API route in `src/app/api/rpc/route.ts` - route to mock when enabled
- [ ] T033 [US3] Add mock transaction state tracking in `src/features/rpc-proxy/lib/mock.ts` - maintain pending/success/reverted states
...
- [ ] T034 [P] [US4] Create rate limiting module in `src/features/rpc-proxy/lib/rate-limit.ts` with Upstash Ratelimit initialization
- [ ] T035 [P] [US4] Implement IP address extraction logic in `src/features/rpc-proxy/lib/rate-limit.ts` - handle X-Forwarded-For and X-Real-IP headers
- [ ] T036 [US4] Configure sliding window algorithm in `src/features/rpc-proxy/lib/rate-limit.ts` - 100 requests per 60 seconds
- [ ] T037 [US4] Add rate limit check function in `src/features/rpc-proxy/lib/rate-limit.ts` - return boolean and remaining quota
- [ ] T038 [US4] Implement fail-open fallback in `src/features/rpc-proxy/lib/rate-limit.ts` - allow requests if KV is unreachable
- [ ] T039 [US4] Integrate rate limiting into Edge API route in `src/app/api/rpc/route.ts` - check before proxying
- [ ] T040 [US4] Add HTTP 429 response with Retry-After header in `src/app/api/rpc/route.ts`
- [ ] T041 [US4] Add rate limit headers to responses (X-RateLimit-Limit, X-RateLimit-Remaining) in `src/app/api/rpc/route.ts`
...
- [ ] T042 [P] Add comprehensive error handling for edge cases across all modules - ensure all HTTP status codes from FR-014 are implemented (429, 503, 400, 500)
- [ ] T043 [P] Validate environment variables on startup in `src/features/rpc-proxy/lib/config.ts` - fail fast with clear error messages
- [ ] T044 [P] Add TypeScript strict mode compliance check for all RPC proxy files
- [ ] T045 Update Wagmi configuration example in application code to use `/api/rpc` endpoint
- [ ] T046 Validate quickstart.md scenarios - test all curl examples and Wagmi integration
- [ ] T047 [P] Add JSDoc comments to all public functions in `src/features/rpc-proxy/lib/`
- [ ] T048 Security audit - verify no API keys in client bundle, no logging of sensitive data
- [ ] T049 Performance validation - measure failover latency, verify <2s threshold
- [ ] T050 Create `.env.local.example` with all required environment variables documented
