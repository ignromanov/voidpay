 ...
 *
 * Unpacks binary-encoded invoice back to InvoiceSchemaV2
 */
...
/**
 * Decodes a Base62-encoded binary invoice back to InvoiceSchemaV2
 */
export function decodeBinary(encoded: string): InvoiceSchemaV2 {
 ...
 */
export function decodeBinaryWithSteps(encoded: string): {
  ...
  addStep(
    ...
    issBytes,
    `${iss} (${new Date(iss * 1000).toISOString()})`
  )
  ...
  addStep(
    ...
    dueBytes,
    `${due} (${new Date(due * 1000).toISOString()})`
  )
