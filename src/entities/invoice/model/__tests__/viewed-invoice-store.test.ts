...
import { useInvoiceViewStore, type ViewedInvoice } from '../viewed-invoice-store'

describe('useInvoiceViewStore', () => {
  beforeEach(() => {
    // Reset store state
    useInvoiceViewStore.setState({
      version: 1,
      invoices: [],
    })
  })

  const createMockInvoice = (overrides: Partial<ViewedInvoice> = {}): Omit<ViewedInvoice, 'viewedAt'> => ({
    invoiceId: 'INV-001',
  ...
  describe('initial state', () => {
    it('initializes with empty invoices array', () => {
      const state = useInvoiceViewStore.getState()
  ...
  describe('addInvoice', () => {
    it('adds invoice to store', () => {
      const { addInvoice } = useInvoiceViewStore.getState()
    ...
    it('sets viewedAt timestamp', () => {
      ...
      const state = useInvoiceViewStore.getState()
      expect(state.invoices[0].viewedAt).toBeDefined()
      expect(new Date(state.invoices[0].viewedAt).getTime()).not.toBeNaN()
    })
  ...
  describe('updateStatus', () => {
    it('updates invoice status', () => {
      const { addInvoice, updateStatus } = useInvoiceViewStore.getState()
    ...
    it('supports all status types', () => {
      ...
      for (const status of statuses) {
        ...
        expect(useInvoiceViewStore.getState().invoices[0].status).toBe(status)
      }
    })
  })
  ...
  describe('setTxHash', () => {
    it('sets transaction hash and marks as paid', () => {
      const { addInvoice, setTxHash } = useInvoiceViewStore.getState()
  ...
  describe('removeInvoice', () => {
    it('removes invoice by invoiceId', () => {
      const { addInvoice, removeInvoice } = useInvoiceViewStore.getState()

      addInvoice(createMockInvoice({ invoiceId: 'TO-REMOVE' }))
      expect(useInvoiceViewStore.getState().invoices).toHaveLength(1)
  ...
  describe('getInvoice', () => {
    it('returns invoice by invoiceId', () => {
      const { addInvoice, getInvoice } = useInvoiceViewStore.getState()
    ...
    it('returns undefined for non-existent invoiceId', () => {
      const { getInvoice } = useInvoiceViewStore.getState()
  ...
  describe('clearAll', () => {
    it('removes all invoices', () => {
      const { addInvoice, clearAll } = useInvoiceViewStore.getState()
      ...
      expect(useInvoiceViewStore.getState().invoices).toHaveLength(3)
  ...
  describe('edge cases', () => {
    ...
    it('handles rapid operations', () => {
      const { addInvoice, updateStatus, setTxHash, removeInvoice } = useInvoiceViewStore.getState()
    ...
    it('preserves data integrity across operations', () => {
      const { addInvoice, updateStatus, setTxHash, getInvoice } = useInvoiceViewStore.getState()
