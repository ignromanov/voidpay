  ...
  return {
    ...
    footer: {
      text: [
        ...
        { text: 'voidpay.xyz', color: COLORS.muted, link: 'https://voidpay.xyz' },
      ],
      alignment: 'center',
      fontSize: 7.5,
      margin: [0, 15, 0, 0],
...
export function buildFilename(data: PartialInvoice): string {
  ...
  // Use total WITHOUT magic dust for filename (display amount, not payment amount)
  const rawTotal = data.total && data.magicDust
...
function partyColumn(
  ...
  if (party.email) {
    lines.push(detailRow(ICONS.mail, String(party.email), align, { link: `mailto:${party.email}` }))
  }
  if (party.phone) {
    lines.push(detailRow(ICONS.phone, String(party.phone), align, { link: `tel:${party.phone}` }))
  }
  if (party.taxId) {
    lines.push(detailRow(ICONS.hash, String(party.taxId), align))
  }

  // Physical address — extra top spacing (matching web mt-3 + MapPinIcon)
  if (party.physicalAddress) {
    lines.push(detailRow(ICONS.mapPin, String(party.physicalAddress), align, {
      color: COLORS.muted,
      lineHeight: 1.5,
      margin: [0, 6, 0, 0],
    }))
  ...
  // Client wallet — full address for financial records (matching web mt-3 + WalletIcon)
  if (label === 'BILL TO' && party.walletAddress) {
    lines.push(detailRow(ICONS.wallet, String(party.walletAddress), align, {
      fontSize: 7,
...
function buildPaymentInfoCard(
  ...
  // Token: icon + symbol, contract address as sub-line
  if (currency) {
    cardContent.push({
      columns: [
